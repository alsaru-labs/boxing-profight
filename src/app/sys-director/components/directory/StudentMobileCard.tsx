...
export function StudentMobileCard({
  ...
  return (
    <div
      className={`
        group relative rounded-2xl p-3 transition-all duration-300
    ...
    >
      <div className="flex justify-between items-start gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h3 className={`text-white font-bold leading-tight text-base tracking-tight mb-0 break-words whitespace-normal ${(student.status === 'Baja' || student.is_active === false) ? 'opacity-50' : ''}`}>
            {student.name} {student.last_name || ""}
          </h3>
          <p className={`text-sky-400/80 text-[11px] truncate font-medium ${(student.status === 'Baja' || student.is_active === false) ? 'opacity-40' : ''}`}>
            {student.email}
          </p>
        </div>
        ...
        <div className="shrink-0 flex flex-col gap-1.5">
          {student.phone && (
              <a
                ...
                className="flex items-center justify-center h-9 w-9 rounded-xl bg-[#25D366]/10 text-[#25D366] border border-[#25D366]/20 transition-all active:scale-95 hover:bg-[#25D366]/20 shadow-lg shadow-[#25D366]/5"
                title="WhatsApp"
              >
      ...
      <div className="flex items-center justify-between pt-2 mt-2 border-t border-white/[0.03]">
