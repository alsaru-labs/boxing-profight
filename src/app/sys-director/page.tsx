...
export default function AdminDashboard() {
  ...
  const {
    ...
    setMonthlyRevenue, setTotalStudents,
  ...
  const handleSaveProfileUI = async (...args: any[]) => {
    const res = await handleSaveProfile(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
    if (res.success) {
  ...
  const sortedClassesList = [...classesList].filter(cls => {
    try {
      ...
      const [hours, minutes] = startTime.split(":").map(Number);
      const classDateTime = new Date(year, month - 1, day, hours, minutes);
      return classDateTime >= now && classDateTime.getTime() <= now.getTime() + (10 * 24 * 60 * 60 * 1000);
    } catch { return false; }
  }).sort((a, b) => {
          ...
          <TabsContent value="general" className="space-y-4 md:space-y-6 focus-visible:outline-none">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4 mb-3 md:mb-6 bg-zinc-900/40 p-3 md:p-5 rounded-2xl border border-white/5">
                <div className="space-y-1">
                  <h3 className="text-base md:text-lg font-black uppercase tracking-widest text-white leading-none mb-1">Periodo de Gestión</h3>
                  <p className="text-white/60 text-sm font-medium leading-tight mt-1 md:mt-0.5">Consulta y valida pagos de cualquier mes.</p>
                </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button
                    ...
                    className="group relative bg-white hover:bg-zinc-100 text-black font-black uppercase tracking-[0.2em] text-[10px] h-12 px-8 rounded-xl transition-all duration-150 hover:scale-[1.03] active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-white/20 border-b-4 border-zinc-300 hover:border-zinc-400 overflow-hidden"
                  >
